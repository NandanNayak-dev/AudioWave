      ...
      >
        ...
        {songsData?.data?.popular && (
          <div className='flex flex-col'>
            <SongList
              songs={showAllSongs ? songsData?.data?.popular?.results : songsData?.data?.popular?.results?.slice(0, 5)}
              type='artist'
            />
            {songsData?.data?.popular?.results?.length > 5 && (
              <button
                onClick={() => setShowAllSongs(!showAllSongs)}
                className='ml-5 mt-4 text-sm font-bold text-gray-400 hover:text-white self-start transition-colors'
              >
